...
class AnnaSource:
        ...
        try:
            ...
            if "/slow_download/" in url:
                ...
                with _DOWNLOAD_URL_CACHE_LOCK:
                    _DOWNLOAD_URL_CACHE[url] = final_url
            ...
            if "momot.rs" in url:
                # Check cache first
                with _DOWNLOAD_URL_CACHE_LOCK:
                    if url in _DOWNLOAD_URL_CACHE:
                        cached_url = _DOWNLOAD_URL_CACHE[url]
                        logger.debug("Using cached download URL for %s", url)
                        return self._fetch_with_retries(cached_url, _headers, stream, is_download=True)
    ...
    def _fetch_with_retries(self, url: str, headers: Dict[str, str], stream: bool, is_download: bool = False) -> Union[requests.Response, _FakeResponse]:
        """
        Fetch a URL with retry logic. For 403 errors on momot.rs, attempt Cloudflare bypass.
        ...
        for attempt in range(MAX_DOWNLOAD_RETRIES):
            try:
                ...
                # Check for Cloudflare challenge on direct momot.rs downloads
                if resp.status_code == 403 and "momot.rs" in url:
                    logger.warning("HTTP 403 on momot.rs (attempt %d/%d), attempting Cloudflare bypass", attempt + 1, MAX_DOWNLOAD_RETRIES)
                    
                    # Try stealth browser bypass
                    final_url = resolve_slow_download_link(url, self.timeout)
                    if final_url:
                        logger.debug("Cloudflare bypass succeeded, retrying download with resolved URL")
                        resp = self.session.get(
                            final_url,
                            headers=headers,
                            timeout=self.timeout,
    ...
    def _safe_get(self, href: str, for_download: bool = False) -> Optional[requests.Response]:
        ...
        if resp.status_code == 429:
            logger.warning(
                "Received 429 Too Many Requests for href=%s; backing off for 5 seconds",
                href,
            )
            resp.close()
            time.sleep(5)  # Back off for 5 seconds on 429
            return None
    ...
    def _is_cloudflare_challenge(self, resp: requests.Response) -> bool:
        ...
        # HTTP 403/503 are strong indicators (error status codes)
        if resp.status_code in {403, 503}:
            return True

        # Check for explicit Cloudflare headers (most reliable)
        ...
        # Only check text content if we have an error status (to avoid false positives)
        # Status 200 with search results may incidentally contain "cloudflare" text
        if resp.status_code in {403, 503}:
            indicators = [
                "just a moment",
                "attention required",
