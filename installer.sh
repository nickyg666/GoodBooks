...
#
# NEW: This installer now supports the package-based installation!
# Run with --package flag to install as a Python package:
#   ./installer.sh --package
#
# Or use pip directly:
#   pip install .
#   goodbooks
...
if [[ "$1" == "--package" ]]; then
    log_info "Installing as Python package..."
    ...
    log_info "Installing GoodBooks package..."
    ...
    log_info "To run: source venv/bin/activate && goodbooks"
    log_info "Or: ./venv/bin/goodbooks"
    
    exit 0
fi

# Get script directory and work directory
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORK_DIR="$(pwd)"  # Current working directory where installer script is run from
INSTALL_DIR="/usr/local/bin/GoodBooks"
VENV_DIR="$INSTALL_DIR/venv"
SERVICE_NAME="goodbooks"
SERVICE_FILE="/etc/systemd/system/${SERVICE_NAME}.service"

# Use work directory if script directory is /usr/local/bin/GoodBooks (fresh install)
if [ "$SCRIPT_DIR" = "/usr/local/bin/GoodBooks" ]; then
    SOURCE_DIR="$WORK_DIR"
else
...
log_info "Copying project files from $SOURCE_DIR..."
...
for script in setup_wizard.sh post_install.py goodreads_epub_utils.py; do
    if [ -f "$SOURCE_DIR/$script" ]; then
        cp "$SOURCE_DIR/$script" "$BUILD_DIR/$script"
...
# For package installation, use: goodbooks
# For legacy installation, use: app.py
...
📦 New Package Installation:
   • Run: ./installer.sh --package
   • Or: pip install . && goodbooks
   • Package mode uses proper Python packaging with venv isolation
