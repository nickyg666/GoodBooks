...
**DO THIS EVERY TIME YOU NEED TO RESTART THE SERVICE:**
```bash
sleep 2 && sudo systemctl restart GoodBooks
```

**WHY**: 
- The `sleep 2` ensures old processes are fully cleaned up before systemctl tries to start
- Without it, port 5000 may still be in use and systemctl will fail to bind
- Always use `sudo systemctl restart` - it properly handles xvfb-run wrapper
- DO NOT use: `pkill python3 app.py && ... python3 app.py`
  - This bypasses the service file's xvfb-run wrapper
  - Cloudflare stealth browser won't work without xvfb
  - Service becomes inconsistent with what systemctl expects

**VERIFY THE RESTART WORKED:**
```bash
sudo systemctl status GoodBooks | head -15
```

Should show:
- `Active: active (running)`
- `Main PID: XXXXX (xvfb-run)`
- `Memory: X.XG` (around 1GB)
- No "failed" messages

## Alternative: Service Management (If You Need to Know)
